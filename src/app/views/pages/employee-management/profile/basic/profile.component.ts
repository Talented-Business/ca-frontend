...
// Services and Models
import { 
	EmployeesService, selectEmployeesBackProcessingFailed,
...
@Component({
	...
	selector: 'kt-employee-basic-profile',
...
export class BasicUserProfileComponent implements OnInit, OnDestroy {
	...
	createForm() {
		this.userForm = this.userFB.group({
			...
			home_phone_number: [this.loginedUser.member.home_phone_number, Validators.required],
			mobile_phone_number: [this.loginedUser.member.mobile_phone_number, [Validators.required]],
			current_password: [""],
			new_password: [""],
			confirm_password: [""],
		});
	...
	prepareUser(password:boolean) {
		...
		_user.home_phone_number = controls['home_phone_number'].value;
		_user.mobile_phone_number = controls['mobile_phone_number'].value;
		if(password){
	...
	updateUser(_user: any) {
		this.employeeService.updateUser(_user).pipe().subscribe(
			(res:any)=>{
				if(res.status=="ok"){
