...
export class MenuHorizontalComponent implements OnInit, AfterViewInit {
	...
	ngOnInit(): void {
		...
		this.menuHorService.menuList$.subscribe(
			(list)=>{
				this.menuList = list;
	...
	loadMenu():void{
		let menus= this.menuConfigService.getMenus();
		if(menus!=undefined){
			this.menuList = menus.header.items;
	...
	checkMenuList():boolean{
		return Array.isArray(this.menuList);
	}
	/**
